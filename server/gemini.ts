...
export async function generateMathQuestion(difficulty: number): Promise<MathQuestion> {
...
以下のJSON形式で回答してください：
{
  ...
  try {
    ...
    const questionData = JSON.parse(jsonMatch[0]) as MathQuestion;
    return questionData;
  } catch (error) {
...
export async function generateLanguageQuestion(difficulty: number): Promise<LanguageQuestion> {
  const model = genAI.getGenerativeModel({ model: 'gemini-2.0-flash-exp' });
...
以下のJSON形式で回答してください：
{
  ...
  try {
    ...
    const questionData = JSON.parse(jsonMatch[0]) as LanguageQuestion;
    return questionData;
  } catch (error) {
    console.error('Error generating language question:', error);
    throw new Error('Failed to generate language question');
  }
}